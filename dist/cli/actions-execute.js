...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === "--input") {
      const v = argv[++i];
      if (v === void 0) return { error: "--input requires a value" };
      inputJson = v;
    } else if (a === "--stdin") {
      stdinFlag = true;
  ...
  if (!actionId)
    return { error: "missing action id. usage: memlin actions execute <id> --input <json>" };
  if (!/^[0-9a-f-]{36}$/i.test(actionId)) {
    return { error: `action id should be a uuid (got "${actionId}")` };
  }
  if (inputJson === null) {
  ...
  if (!inputJson) {
    return {
      error: "no input provided. Pass --input '<json>' or pipe JSON via stdin"
    };
  }
  return { actionId, inputJson, json };
}
function printHelp() {
  console.log(
    [
      "memlin actions execute \u2014 invoke a callable action",
      "",
      "Usage:",
      "  memlin actions execute <id> --input '<json>'",
      "  echo '<json>' | memlin actions execute <id>",
      "",
      "Options:",
      "  --input <json>   Input object matching the action's input_schema",
      "  --stdin          Read input from stdin (auto-detected when piped)",
      "  --json           Emit raw JSON response instead of the readable summary",
      "",
      "Find action ids with: memlin actions list"
    ].join("\n")
  );
}
async function main() {
  ...
  if ("error" in parsed) {
    ...
    console.error(`memlin actions execute: ${parsed.error}`);
    printHelp();
    process.exit(2);
  }
  let input;
  try {
    const v = JSON.parse(parsed.inputJson);
    if (!v || typeof v !== "object" || Array.isArray(v)) {
      console.error("memlin actions execute: --input must be a JSON object");
      process.exit(2);
    }
    input = v;
  } catch (e) {
    console.error(`memlin actions execute: invalid JSON \u2014 ${e instanceof Error ? e.message : e}`);
    process.exit(2);
  }
  ...
  let result;
  try {
    result = await ctx.api.executeAction(parsed.actionId, input);
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e);
    if (/invalid_input|input failed validation/i.test(msg)) {
      console.error(`memlin actions execute: ${msg}`);
      process.exit(3);
    }
    console.error(`memlin actions execute failed: ${msg}`);
    process.exit(1);
  }
  if (parsed.json) {
  ...
  const metaLine = [
    ...
    `${m.latency_ms}ms`,
    m.input_tokens !== void 0 ? `${m.input_tokens}\u2192${m.output_tokens ?? 0} tokens` : null
  ].filter(Boolean).join(" \xB7 ");
  console.log(`# ${metaLine}`);
  console.log(`# audit_id: ${result.audit_id || "(not recorded)"}`);
  console.log("");
  process.stdout.write(result.output);
  if (!result.output.endsWith("\n")) process.stdout.write("\n");
}
main().catch((err) => {
  console.error("memlin actions execute failed:", err instanceof Error ? err.message : err);
  process.exit(1);
});
