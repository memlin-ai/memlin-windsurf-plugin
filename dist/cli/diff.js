...
function diffContract(contract, local, mode = "subset") {
  const drift = [];
  function walk(c, l, path7) {
    if (c === null || typeof c !== "object" || Array.isArray(c)) {
      if (!deepEq(c, l)) drift.push({ path: path7, contract: c, local: l, reason: "mismatch" });
      return;
    }
    const lObj = l && typeof l === "object" && !Array.isArray(l) ? l : null;
    for (const [k, cv] of Object.entries(c)) {
      const nextPath = path7 ? `${path7}.${k}` : k;
      if (!lObj || !(k in lObj)) {
...
async function main() {
  ...
  let localRaw;
  try {
    localRaw = await fs5.readFile(localFile, "utf8");
  } catch (e) {
