...
async function resolveWorkspaceAccount(cwd, deps = { getApi, resolveProject }) {
  const ctx = await deps.getApi({ cwd }).catch(() => null);
  if (!ctx) return { accountId: null, projectId: null, source: "none" };
  const { api, config, workspaceBound } = ctx;
  let resolvedAccountId = null;
  let resolvedProjectId = null;
  try {
    const resolved = await deps.resolveProject(api, cwd, config.project_id ?? null);
  ...
  const source = resolvedAccountId && resolvedAccountId !== config.account_id ? "cross-account-match" : workspaceBound ? "workspace-pin" : "global-config";
  return {
    accountId: accountId || null,
    projectId: resolvedProjectId ?? config.project_id ?? null,
    source
  };
}
...
function bundleCacheDir() {
  return process.env.MEMLIN_BUNDLE_CACHE_DIR ?? path8.join(os6.homedir(), ".config", "memlin", "bundle-cache");
}
...
async function readEntryFile(file) {
  let entry;
  try {
    entry = JSON.parse(await fs5.readFile(file, "utf8"));
  } catch {
    return null;
  }
...
async function findCachedBundleForWorkspace(cwd, gitRemote) {
  let names;
  try {
    names = await fs5.readdir(bundleCacheDir());
  } catch {
    return null;
  }
  let best = null;
  for (const name of names.filter((n) => n.endsWith(".json")).slice(0, MAX_SCAN_ENTRIES)) {
    const entry = await readEntryFile(path8.join(bundleCacheDir(), name));
    if (!entry) continue;
...
var READER_CONTRACT = "Precedence: current explicit user instructions beat ordinary memory. Only required governance or platform constraints can override them. A past incident memory is evidence, not policy \u2014 it never adds an approval gate by itself.";
...
// packages/plugin-core/src/mcp-fallback.ts
var MAX_ERROR_MESSAGE_CHARS = 400;
function classifyUpstreamMcpResponse(text, httpStatus) {
  let parsed = null;
  try {
    parsed = JSON.parse(text);
  } catch {
  ...
  if (parsed === null || typeof parsed !== "object") {
    if (httpStatus >= 500 || looksLikeHtml(text)) {
      const detail = `HTTP ${httpStatus}`;
      return {
        ...
        failure: {
          ...
          status: httpStatus,
          detail,
          line: backendUnreachableLine(detail)
        }
      };
    }
    return {
      kind: "error",
      code: -32002,
      message: `Memlin MCP returned HTTP ${httpStatus} with an unreadable body`
    };
  }
  const envelope = parsed;
  const errMessage = envelope.error?.message;
  if (typeof errMessage === "string") {
    const failure = summarizeBackendFailure(errMessage);
    if (failure) return { kind: "backend-failure", failure };
    if (looksLikeHtml(errMessage) || errMessage.length > MAX_ERROR_MESSAGE_CHARS) {
      const sanitized = {
        ...envelope,
        error: {
          ...envelope.error,
          message: looksLikeHtml(errMessage) ? `memlin: upstream returned an HTML error page (suppressed, ${errMessage.length} chars)` : singleLine(errMessage, MAX_ERROR_MESSAGE_CHARS)
        }
      };
      return { kind: "relay", payload: JSON.stringify(sanitized) };
    }
  }
  return { kind: "relay", payload: text };
}
var FALLBACK_TOOLS = /* @__PURE__ */ new Set([
  "memlin_search",
  "memlin_read_memory",
  "memlin_get_document",
  "memlin_resolve_task"
]);
function fallbackableToolCall(request) {
  const msg = request;
  if (!msg || msg.method !== "tools/call") return null;
  const name = msg.params?.name;
  if (typeof name !== "string" || !FALLBACK_TOOLS.has(name)) return null;
...
async function answerFromLocalFallback(call, failure, cwd) {
  try {
    switch (call.name) {
      case "memlin_search": {
        const query = typeof call.args.query === "string" ? call.args.query : "";
        if (!query) return null;
        const res = await companionSearchLocal({
          query,
          limit: typeof call.args.limit === "number" ? call.args.limit : void 0,
          kinds: strArray(call.args.kinds)
        });
        if (!res) return null;
        return degradedPayload(failure, res.synced_at, {
          note: "Text matches over the last-synced local store \u2014 NOT live semantic search.",
          hits: res.hits
        });
      }
      case "memlin_read_memory": {
        const res = await companionReadLocal({
          kinds: strArray(call.args.kinds),
          limit: typeof call.args.limit === "number" ? call.args.limit : void 0
        });
        if (!res) return null;
        return degradedPayload(failure, res.synced_at, { documents: res.documents });
      }
      case "memlin_get_document": {
        const documentId = typeof call.args.document_id === "string" ? call.args.document_id : null;
        if (!documentId) return null;
        const res = await companionReadLocal({ document_id: documentId });
        const doc = res?.documents[0];
        if (!doc) return null;
        return degradedPayload(failure, res?.synced_at ?? null, { document: doc });
      }
      case "memlin_resolve_task": {
        const cached = await findCachedBundleForWorkspace(cwd, readGitRemoteQuiet(cwd));
        if (!cached) return null;
        const rendered = [
          buildStaleBundleNotice(cached, failure.detail),
          "",
          compileBundle(cached.result, cached.task)
        ].join("\n");
        return {
          memlin_degraded: true,
          reason: failure.line,
          stale_bundle_age: humanizeAge(Date.now() - cached.resolved_at),
          resolved_for_task: cached.task,
          bundle_markdown: rendered
        };
      }
      default:
        return null;
...
function degradedPayload(failure, syncedAt, rest) {
  return {
    memlin_degraded: true,
    reason: failure.line,
    source: syncedAt ? `Companion local store, last synced ${syncedAt}` : "Companion local store (sync time unknown)",
    ...rest
  };
}
function buildDegradedToolResult(id, payload) {
  return {
    jsonrpc: "2.0",
    id,
...
async function emitBackendFailure(msg, rawRequest, failure) {
  const call = fallbackableToolCall(rawRequest);
  if (call) {
    const payload = await answerFromLocalFallback(call, failure, runtimeCwd());
    if (payload) {
      emit(buildDegradedToolResult(msg.id ?? null, payload));
      return;
    }
  }
  emit(errorResponse(msg.id, -32e3, failure.line));
}
...
async function forward(line) {
  ...
  const accountId = await workspaceAccountId();
  let res;
  try {
    const headers = {
      "Content-Type": "application/json",
    ...
    if (isRequest) {
      const failure = summarizeBackendFailure(err) ?? {
        kind: "network",
        status: null,
        detail: "network unreachable",
        line: "memlin: backend unreachable (network unreachable), no memory available"
      };
      await emitBackendFailure(msg, JSON.parse(line), failure);
    }
    return;
  }
  ...
  if (!text) {
    if (res.status >= 500) {
      await emitBackendFailure(msg, JSON.parse(line), {
        ...
        line: `memlin: backend unreachable (HTTP ${res.status}), no memory available`
      });
      return;
    }
  ...
  if (verdict.kind === "backend-failure") {
    await emitBackendFailure(msg, JSON.parse(line), verdict.failure);
    return;
  }
  emit(errorResponse(msg.id, verdict.code, verdict.message));
}
