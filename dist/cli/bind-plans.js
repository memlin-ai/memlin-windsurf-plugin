...
async function listUnboundPlans() {
  ...
  let entries;
  try {
    entries = await fs5.readdir(plansDir());
  } catch {
    return out;
  }
  const state = await readState();
  for (const f of entries) {
    ...
    const abs = path7.join(plansDir(), f);
    let raw;
    let size = 0;
    try {
    ...
    const relPath = path7.relative(homeBase(), abs);
...
// packages/plugin-core/src/cli/bind-plans.ts
var PLANS_DIR = path8.join(os6.homedir(), ".claude", "plans");
...
`
  ...
  const targets = all ? unbound.map((p) => p.file) : [fileArg].map((f) => path8.basename(f));
  if (targets.length === 0) {
...
`
  ...
  for (const file of targets) {
    const abs = path8.join(PLANS_DIR, file);
    try {
