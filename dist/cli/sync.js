...
function companionSocketPath(env = process.env) {
  ...
  return path7.join(os4.homedir(), ".config", "memlin", "run", "companion.sock");
}
function companionRunDir() {
  return path7.join(os4.homedir(), ".config", "memlin", "run");
}
...
// packages/plugin-core/src/state.ts
var STATE_FILE = path3.join(os.homedir(), ".config", "memlin", "state.json");
var EMPTY = { documents: {} };
...
var ClaudeCodeHost = class extends BaseHost {
  constructor() {
    super("claude-code", path4.join(os2.homedir(), ".claude"));
  }
};
var CursorHost = class extends BaseHost {
  constructor() {
    super("cursor", path4.join(os2.homedir(), ".config", "memlin"));
  }
};
var CodexHost = class extends BaseHost {
  constructor() {
    super("codex", path4.join(os2.homedir(), ".config", "memlin"));
  }
};
var WindsurfHost = class extends BaseHost {
  constructor() {
    super("windsurf", path4.join(os2.homedir(), ".config", "memlin"));
  }
};
var AntigravityHost = class extends BaseHost {
  constructor() {
    super("antigravity", path4.join(os2.homedir(), ".config", "memlin"));
  }
};
var VSCodeHost = class extends BaseHost {
  constructor() {
    super("vscode", path4.join(os2.homedir(), ".config", "memlin"));
  }
};
var CompanionHost = class extends BaseHost {
  constructor() {
    super("companion", path4.join(os2.homedir(), ".config", "memlin"));
  }
};
var HOSTS = {
...
async function scanLocal(opts = {}) {
  ...
  const memDir = path5.join(root, "memory");
  if (existsSync2(memDir)) {
    for (const file of await fs3.readdir(memDir)) {
      if (!file.endsWith(".md") || file === "MEMORY.md") continue;
      const abs = path5.join(memDir, file);
      const content = await fs3.readFile(abs, "utf8");
      out.push({
        path: `memory/${file}`,
        abs_path: abs,
        kind: "memory",
  ...
  const skillsDir = path5.join(root, "skills");
  if (existsSync2(skillsDir)) {
    const entries = await fs3.readdir(skillsDir, { withFileTypes: true });
    for (const e of entries) {
      if (!e.isDirectory()) continue;
      const skillMd = path5.join(skillsDir, e.name, "SKILL.md");
      if (!existsSync2(skillMd)) continue;
      const content = await fs3.readFile(skillMd, "utf8");
      out.push({
  ...
  const goalsDir = path5.join(root, "goals");
  if (existsSync2(goalsDir)) {
    for (const file of await fs3.readdir(goalsDir)) {
      if (!file.endsWith(".md")) continue;
      const abs = path5.join(goalsDir, file);
      const content = await fs3.readFile(abs, "utf8");
      out.push({
        path: `goals/${file}`,
        abs_path: abs,
  ...
  const schemasDir = path5.join(root, "schemas");
  if (existsSync2(schemasDir)) {
    for (const file of await fs3.readdir(schemasDir)) {
      if (!file.endsWith(".json")) continue;
      const abs = path5.join(schemasDir, file);
      const content = await fs3.readFile(abs, "utf8");
      out.push({
        path: `schemas/${file}`,
        abs_path: abs,
  ...
  if (opts.includePlans) {
    ...
    if (existsSync2(plansDir)) {
      for (const file of await fs3.readdir(plansDir)) {
        if (!file.endsWith(".md")) continue;
        const abs = path5.join(plansDir, file);
        const content = await fs3.readFile(abs, "utf8");
        out.push({
          path: `plans/${file}`,
          abs_path: abs,
  ...
  if (opts.trackedDocs) {
    ...
    for (const [relPath, meta] of Object.entries(opts.trackedDocs)) {
      ...
      const abs = path5.join(root, relPath);
...
function archiveRoot() {
  return path6.join(os3.homedir(), ".config", "memlin", "archive");
}
...
async function applyPullToLocal(docs, state, now, rootOverride) {
  ...
  for (const d of docs) {
    ...
    let needsWrite = true;
    try {
      const local = await fs4.readFile(full, "utf8");
      if (hash(local) === contentHash) needsWrite = false;
    } catch {
    }
    if (needsWrite) {
      await fs4.mkdir(path6.dirname(full), { recursive: true });
      await fs4.writeFile(full, d.content, "utf8");
...
function persistedTokenFilePath() {
  return process.env.MEMLIN_TOKEN_FILE || path8.join(os5.homedir(), ".config", "memlin", "token.json");
}
...
async function acquireAuthFileLock() {
  ...
  await fs5.mkdir(path8.dirname(file), { recursive: true });
  const deadline = Date.now() + AUTH_FILE_LOCK_TIMEOUT_MS;
  while (true) {
    try {
      const handle = await fs5.open(file, "wx", 384);
      try {
        ...
        await fs5.rm(file, { force: true }).catch(() => {
        });
        throw error;
      }
      let released = false;
      return async () => {
        ...
        const currentOwner = await fs5.readFile(file, "utf8").catch(() => null);
        if (currentOwner === owner) await fs5.rm(file, { force: true }).catch(() => {
        });
      };
    } catch (error) {
...
async function writePersistedToken(t) {
  ...
  const tmp = path8.join(
    path8.dirname(file),
    `${path8.basename(file)}.tmp-${process.pid}-${randomUUID()}`
  );
...
async function readSmallRegularFile(file) {
  ...
  try {
    ...
    const handle = await fs6.open(file, constants.O_RDONLY | noFollow);
    try {
      ...
      const [after, afterPath] = await Promise.all([handle.stat(), fs6.lstat(file)]);
...
var AGENT_KINDS = [
  ...
  // Plugin system is Python-only; today this kind reflects MCP-only
  // connections. Phase 2 ships a native Python plugin (apps/hermes-plugin/)
  // wiring pre_tool_call into the same controller path Claude Code uses.
  "hermes",
  // OpenClaw — Peter Steinberger's viral open-source autonomous personal
  // agent (Discord/Slack-first UI, local-first, 247k+ stars by Mar 2026).
  // Has MCP Registry support and a SKILL.md skills format at
  // ~/.openclaw/workspace/skills/. MCP-only today; the Phase 3
  // agentskills.io bridge is the bet for skills round-tripping.
  "openclaw",
  // Google Antigravity — Google's agent-first IDE (Nov 2025), built by the
  // former Windsurf/Codeium team on a VS Code fork. MCP servers live in
  // ~/.gemini/antigravity/mcp_config.json (remote servers use `serverUrl`,
  // exactly Memlin's hosted shape); project rules in .agent/rules/*.md and
...
var DocumentStatusSchema = external_exports.enum(DOCUMENT_STATUSES);
var AgentKindSchema = external_exports.enum(AGENT_KINDS);
var MemberRoleSchema = external_exports.enum(MEMBER_ROLES);
var AccountTierSchema = external_exports.enum(ACCOUNT_TIERS);
var SkillFrontmatterSchema = external_exports.object({
  name: external_exports.string().min(1).max(64),
  description: external_exports.string().min(10).max(1024),
  "allowed-tools": external_exports.array(external_exports.string()).optional(),
  model: external_exports.string().optional(),
  "target-agents": external_exports.array(external_exports.string()).optional(),
  "component-links": external_exports.array(external_exports.string()).optional(),
...
var GoalFrontmatterSchema = external_exports.object({
  ...
  owners: external_exports.array(UUID).default([])
});
var WriteDocumentInputSchema = external_exports.object({
...
var DocumentPatchSchema = external_exports.object({
  ...
  content: external_exports.string(),
  version_number: external_exports.number().int().positive(),
...
var BrandColorSchema = external_exports.object({
  role: external_exports.enum(BRAND_COLOR_ROLES),
  name: external_exports.string().min(1).max(64),
  hex: external_exports.string().regex(
    /^#([0-9a-fA-F]{3}|[0-9a-fA-F]{6}|[0-9a-fA-F]{8})$/,
    "expected #RGB, #RRGGBB, or #RRGGBBAA"
  ),
  // Short free-form description of where the color is used. Imported from
  // CSS trailing comments (`--ink: #0A0B14; /* body text on light */`)
  // and editable in the UI. Flows into resolver bundles so AI clients see
  // "Memlin Ink — body text on light" instead of just a hex value.
  usage: external_exports.string().max(256).optional()
});
var BrandFontSchema = external_exports.object({
  family: external_exports.string().min(1).max(128),
  weights: external_exports.array(external_exports.number().int().min(100).max(900)).default([]),
  source_url: external_exports.string().url().nullable().optional()
});
var BrandLogoSchema = external_exports.object({
  // Path relative to the brand-guidelines-assets bucket:
  // <account>/<brand_guidelines_id>/<filename>. Empty string = slot is
  // intentionally left blank. The web layer mints signed URLs at render
  // time; the resolver does the same for AI bundles.
  storage_path: external_exports.string().max(512),
  alt: external_exports.string().max(256).default("")
});
var BrandImageryNoteSchema = external_exports.object({
  label: external_exports.string().min(1).max(128),
  description: external_exports.string().max(1024).default("")
});
var BrandGuidelinesFrontmatterSchema = external_exports.object({
  name: external_exports.string().min(1).max(128),
  tagline: external_exports.string().max(256).optional(),
  description: external_exports.string().max(2048).optional(),
  colors: external_exports.array(BrandColorSchema).optional(),
  typography: external_exports.object({
    heading: BrandFontSchema.optional(),
    body: BrandFontSchema.optional(),
    mono: BrandFontSchema.optional()
  }).optional(),
  logos: external_exports.object({
    primary_light: BrandLogoSchema.optional(),
    primary_dark: BrandLogoSchema.optional(),
    favicon: BrandLogoSchema.optional()
  }).optional(),
...
var REDACTION_PATTERNS = [
  ...
  },
  {
    ...
    description: "GitHub OAuth user-to-server token (gho_)",
...
for (const p of REDACTION_PATTERNS) {
  if (!p.regex.global) {
    throw new Error(`redact: pattern '${p.name}' must use the 'g' flag`);
  }
}
...
var ProviderCallImplSchema = external_exports.object({
  type: external_exports.literal("provider_call"),
  provider: external_exports.enum(["anthropic", "google", "xai"]),
  /** Anthropic model id (e.g. claude-haiku-4-5, claude-sonnet-4-6).
   *  Validated as a string here; the executor verifies the model is
   *  callable at dispatch time. */
  model: external_exports.string().min(1).max(128),
  /** Mustache-style prompt template; `{{input.field}}` placeholders are
   *  substituted at execute time against the validated input. Plain
   *  string for v1; future versions might accept structured messages. */
  prompt_template: external_exports.string().min(1).max(8e3),
  /** Optional cap on response tokens. Defaults to 1024 in the executor
   *  when omitted, which is plenty for the summarization / ask
   *  patterns Phase 1 supports. */
  max_output_tokens: external_exports.number().int().min(1).max(8e3).optional()
});
var HttpAuthSchema = external_exports.discriminatedUnion("type", [
  ...
  external_exports.object({
    ...
    /** The HTTP header to put the key in (e.g. 'x-api-key'). */
    header_name: external_exports.string().min(1).max(128),
    token_env: external_exports.string().min(1).max(128).regex(/^[A-Z_][A-Z0-9_]*$/, {
      message: "token_env must be a SCREAMING_SNAKE_CASE env-var name"
    })
  })
]);
var HttpImplSchema = external_exports.object({
  type: external_exports.literal("http"),
  method: external_exports.enum(["GET", "POST", "PUT", "PATCH", "DELETE"]).optional(),
  /** Full URL including scheme. https://* is preferred; http://* is
   *  accepted but the executor warns. Any other scheme rejected by both
   *  the schema and the dispatcher. */
  url: external_exports.string().url().refine((u) => /^https?:\/\//i.test(u), "url must start with https:// or http://"),
  /** Static headers merged with the auth header. Templated values
   *  NOT supported in v1 — too easy to leak data via header
   *  injection. body_template covers the dynamic case. */
  headers: external_exports.record(external_exports.string()).optional(),
  /** Body template — same {{input.x}} mustache substitution as
   *  prompt_template. Sent as-is (no JSON wrapping) so the action
   *  author can craft JSON or form-urlencoded bodies as they need.
   *  Required for POST/PUT/PATCH; ignored for GET/DELETE. */
  body_template: external_exports.string().max(64e3).optional(),
  auth: HttpAuthSchema.optional(),
  /** Hard cap on response body returned to the caller. Default 16KB.
   *  Larger responses are truncated with a `[…truncated]` suffix. */
  max_response_bytes: external_exports.number().int().min(256).max(1048576).optional()
});
var ConnectorImplSchema = external_exports.object({
  type: external_exports.literal("connector"),
  connector_id: external_exports.string().min(1).max(64).regex(/^[a-z][a-z0-9_]*(\.[a-z][a-z0-9_]*)*$/, {
    message: 'connector_id must be lowercase dot.snake_case (e.g. "github.search_issues")'
  }),
  /** Per-connector config — validated against the connector's own
...
var ActionMetadataSchema = external_exports.object({
  /** The action's public name — used in audit rows + future MCP tool ids. */
  name: ActionNameSchema,
  /** One-line description (also goes into MCP tool description in Phase 2). */
  description: external_exports.string().min(1).max(280),
  /** JSON Schema body validating the input the caller passes. We accept
   *  a generic record here; the executor parses it with a JSON Schema
   *  validator at execute time. Keeping it as `record(unknown)` rather
   *  than zod-coerced gives action authors full JSON Schema (oneOf,
   *  enum, format) without us shadowing every feature. */
...
function parseSkill(content) {
  ...
  try {
    ...
    const frontmatter = fm.success ? fm.data : {
      ...
      "allowed-tools": Array.isArray(fmRaw["allowed-tools"]) ? fmRaw["allowed-tools"] : void 0,
      model: typeof fmRaw.model === "string" ? fmRaw.model : void 0,
      "target-agents": Array.isArray(fmRaw["target-agents"]) ? fmRaw["target-agents"] : void 0,
      "component-links": Array.isArray(fmRaw["component-links"]) ? fmRaw["component-links"] : void 0,
      examples: Array.isArray(fmRaw.examples) ? fmRaw.examples : void 0,
      "anti-examples": Array.isArray(fmRaw["anti-examples"]) ? fmRaw["anti-examples"] : void 0
    };
    return {
      frontmatter,
...
// packages/shared/dist/prompt-linter.js
var VAGUE_PHRASES = [
  { phrase: "be professional", suggestion: "Define specific tone guidelines instead." },
  {
    phrase: "try to",
    suggestion: 'Use direct imperative verbs like "Perform", "Ensure" or "Execute".'
  },
  {
    phrase: "don't do stupid things",
    suggestion: "State explicit restrictions or negative examples."
  },
  { phrase: "avoid mistakes", suggestion: "Provide specific verification steps." },
  { phrase: "be smart", suggestion: "State the precise decision criteria instead." },
  { phrase: "handle carefully", suggestion: "Specify the error handling steps." },
  { phrase: "do your best", suggestion: "Define the quantitative success criteria." }
];
function lintPrompt(content) {
  const messages = [];
  ...
  if (!examples || examples.length === 0) {
    messages.push({
      severity: "warning",
      category: "structure",
      message: 'Skill is missing "examples" frontmatter. Add positive examples to guide the agent.'
    });
  }
  const antiExamples = fm["anti-examples"];
  if (!antiExamples || antiExamples.length === 0) {
    messages.push({
      severity: "warning",
      category: "structure",
      message: 'Skill is missing "anti-examples" frontmatter. Add negative anti-examples to prevent agent mistakes.'
    });
  }
  const lines = content.split("\n");
  for (let i = 0; i < lines.length; i++) {
    const lineRaw = lines[i];
    if (lineRaw === void 0) continue;
    ...
    for (const { phrase, suggestion } of VAGUE_PHRASES) {
      if (line.includes(phrase)) {
        messages.push({
          severity: "warning",
          category: "vague",
          message: `Vague language detected: "${phrase}". ${suggestion}`,
          line: i + 1
        });
      }
    }
  ...
  for (let i = 0; i < lines.length; i++) {
    const rawLineRaw = lines[i];
    if (rawLineRaw === void 0) continue;
    const rawLine = rawLineRaw.trim();
    if (rawLine.length < 10) continue;
    const cleanLine = rawLine.toLowerCase();
    if (seenLines.has(cleanLine)) {
      messages.push({
        severity: "warning",
        category: "conflict",
        message: `Duplicate instruction line detected: "${rawLine}"`,
        line: i + 1
      });
    } else {
      seenLines.add(cleanLine);
    }
    if (cleanLine.includes("always") || cleanLine.includes("must")) {
      const verb = cleanLine.replace(/always|must/g, "").trim();
      if (verb.length > 5) {
        for (let j = 0; j < lines.length; j++) {
          const otherRaw = lines[j];
          if (otherRaw === void 0) continue;
          const other = otherRaw.toLowerCase();
          if (i !== j && (other.includes("never") || other.includes("should not")) && other.includes(verb.slice(0, 10))) {
            messages.push({
              severity: "warning",
              category: "conflict",
              message: `Potential conflicting instruction on line ${j + 1}: "${otherRaw.trim()}" contradicts "${rawLine}"`,
              line: i + 1
            });
          }
        }
...
async function main() {
  ...
  console.log(
    `memlin sync \u2014 account ${config.account_id.slice(0, 8)}\u2026 project ${resolved.project_id?.slice(0, 8) ?? "(none)"} (${resolved.reason})`
  );
  ...
  for (const doc of local) {
    ...
    if (prev && prev.content_hash === doc.hash) continue;
    let title = inferTitle(doc.path, doc.content);
    const metadata = {};
    if (doc.kind === "skill") {
      try {
        const parsed = parseSkill(doc.content);
        const lintMessages = lintPrompt(doc.content);
        for (const msg of lintMessages) {
          const prefix = msg.severity === "error" ? "\u2717" : "\u26A0";
          const lineStr = msg.line ? `:${msg.line}` : "";
          console.warn(`  \x1B[33m${prefix} Lint [${doc.path}${lineStr}]: ${msg.message}\x1B[0m`);
        }
        if (parsed.frontmatter.name) {
          title = parsed.frontmatter.name;
        }
        if (parsed.frontmatter["allowed-tools"]?.length) {
          metadata["allowed-tools"] = parsed.frontmatter["allowed-tools"];
        }
        if (parsed.frontmatter.model) {
          metadata.model = parsed.frontmatter.model;
        }
        if (parsed.frontmatter["target-agents"]?.length) {
          metadata["target-agents"] = parsed.frontmatter["target-agents"];
        }
        if (parsed.frontmatter["component-links"]?.length) {
          metadata["component-links"] = parsed.frontmatter["component-links"];
        }
        if (parsed.frontmatter.examples?.length) {
          metadata.examples = parsed.frontmatter.examples;
        }
        if (parsed.frontmatter["anti-examples"]?.length) {
          metadata["anti-examples"] = parsed.frontmatter["anti-examples"];
        }
      } catch (err) {
        console.warn(`  \u26A0 failed to parse frontmatter for ${doc.path}:`, err);
      }
    }
    const result2 = await api.writeDocument({
    ...
    pushed++;
    console.log(`  \u2191 ${doc.path} (v${result2.version_number})`);
  }
  const m1 = await api.listDocuments({
    kinds: ["memory", "skill"],
    project_id: resolved.project_id
  });
  const m2 = await api.listDocuments({
    kinds: ["goal"],
    statuses: ["approved"],
    project_id: resolved.project_id
  });
