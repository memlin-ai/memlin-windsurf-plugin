...
async function findLatestTranscript(cwd) {
  ...
  const direct = path8.join(projectsDir, encoded);
  let scanDirs = [];
  try {
    const st = await fs5.stat(direct);
    if (st.isDirectory()) scanDirs.push(direct);
  } catch {
    try {
      const dirs = await fs5.readdir(projectsDir);
      scanDirs = dirs.map((d) => path8.join(projectsDir, d));
    } catch {
      return null;
    }
  }
...
async function loadTranscript(transcriptPath) {
  const raw = await fs5.readFile(transcriptPath, "utf8");
...
async function main() {
  ...
  let found;
  if (explicitFile) {
    const abs = path8.resolve(cwd, explicitFile);
  ...
  try {
    ...
    if (explicitFile && transcript.length === 0) {
      const raw = await fs5.readFile(found.path, "utf8");
      transcript = `### user
${raw}`;
    }
  } catch (err) {
