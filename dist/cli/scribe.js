...
async function findLatestTranscript(cwd) {
  const projectsDir = path7.join(os6.homedir(), ".claude", "projects");
  const encoded = cwd.replace(/\//g, "-");
  const direct = path7.join(projectsDir, encoded);
  let scanDirs = [];
  try {
    const st = await fs4.stat(direct);
    if (st.isDirectory()) scanDirs.push(direct);
  } catch {
    try {
      const dirs = await fs4.readdir(projectsDir);
      scanDirs = dirs.map((d) => path7.join(projectsDir, d));
    } catch {
      return null;
    }
  }
...
async function loadTranscript(transcriptPath) {
  const raw = await fs4.readFile(transcriptPath, "utf8");
...
async function main() {
  ...
  let found;
  if (explicitFile) {
    const abs = path7.resolve(cwd, explicitFile);
...
`
  ...
  try {
    const result = await ctx.api.scribeSession({
      session_id: found.sessionId,
      transcript
    });
    if (result.proposals_persisted === 0) {
      process.stdout.write(
        `done. ${result.proposals_extracted} extracted, ${result.proposals_persisted} persisted (run ${result.run_id.slice(0, 8)}, ${result.latency_ms}ms).
