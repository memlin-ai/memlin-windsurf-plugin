...
function printHelp() {
  console.log(
    [
      "memlin pin \u2014 force-include a document into every resolved bundle in scope",
      "",
      "Usage:",
      "  memlin pin <document-id>          force-include the document",
      "  memlin pin <document-id> --off    remove the pin (unpin)",
      "",
      "A pin bypasses similarity ranking and reserves budget off the top, so the",
      "document appears as a STANDING DIRECTIVE at the top of every bundle in scope.",
      "Owner/admin-only."
    ].join("\n")
  );
}
async function main() {
  ...
  const pinned = !argv.includes("--off") && !argv.includes("--unpin");
  const ctx = await getApi();
  if (!ctx) {
    console.error("memlin pin: not configured. Run `memlin login` first.");
    process.exit(1);
  }
  const { api } = ctx;
  try {
    const res = await api.setDocumentPinned({ document_id: documentId, pinned });
    console.log(
      res.pinned ? `Pinned ${res.document_id} \u2014 it will now force-include into every bundle in scope.` : `Unpinned ${res.document_id} \u2014 it returns to normal similarity-ranked retrieval.`
    );
  } catch (err) {
    console.error(`memlin pin: ${err instanceof Error ? err.message : err}`);
    process.exit(1);
  }
}
main().catch((err) => {
  console.error("memlin pin failed:", err instanceof Error ? err.message : err);
  process.exit(1);
});
