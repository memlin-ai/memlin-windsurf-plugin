...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === "--filter") {
      const v = argv[++i];
      if (!v) return { error: "--filter requires a value" };
      filter = v;
    } else if (a === "--limit") {
      const v = argv[++i];
      if (!v) return { error: "--limit requires a value" };
      const n = Number(v);
      if (!Number.isFinite(n) || n <= 0) return { error: `--limit: bad number "${v}"` };
      limit = Math.floor(n);
...
function printHelp() {
  console.log(
    [
      "memlin actions list \u2014 list callable actions in this workspace",
      "",
      "Usage:",
      "  memlin actions list [--filter <substring>] [--limit <n>] [--json]",
      "",
      "Options:",
      "  --filter <s>    Case-insensitive substring match on action name",
      "  --limit <n>     Cap result count (default 50, max 200)",
      "  --json          Emit raw JSON instead of the table",
      "",
      "See also:",
      "  memlin actions execute <id> \u2014 invoke an action"
    ].join("\n")
  );
}
function renderRow(a) {
  const required = Array.isArray(a.input_schema.required) ? a.input_schema.required ?? [] : [];
  const inputSummary = required.length > 0 ? `{${required.join(", ")}}` : "{}";
  return [
    `${a.name}  (${a.implementation_type})`,
    `  id:           ${a.id}`,
