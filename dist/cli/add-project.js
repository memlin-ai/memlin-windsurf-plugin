...
function childGitRemotes(cwd, deps = {}) {
  ...
  const readRemote = deps.readRemote ?? ((repoPath) => {
    try {
      if (!existsSync2(path8.join(repoPath, ".git"))) return null;
  ...
  for (const dir of listDirs(cwd).slice(0, MAX_CHILD_DIRS)) {
    const remote = readRemote(path8.join(cwd, dir));
...
function defaultUserSettingsPaths() {
  const claudeDir = path9.join(os6.homedir(), ".claude");
...
Attach this folder to it instead of creating a new project? [Y/n] `,
  ...
  const projectName = parsed.name?.trim() || path10.basename(cwd).trim() || "untitled";
  let project;
  try {
