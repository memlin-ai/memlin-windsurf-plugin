...
function gatePath(trajectoryId) {
  ...
  return path9.join(os7.tmpdir(), `memlin-windsurf-session-start-${key}`);
}
async function alreadyRan(trajectoryId) {
  try {
    await fs6.access(gatePath(trajectoryId));
    return true;
  } catch {
    return false;
  }
...
async function markRan(trajectoryId) {
  try {
    await fs6.writeFile(gatePath(trajectoryId), JSON.stringify({ at: Date.now() }), "utf8");
  } catch {
  }
}
