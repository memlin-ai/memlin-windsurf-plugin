...
async function evaluateEditCollision(ctx, payload, projectId, projectAccountId) {
  ...
  const relPaths = [
    ...new Set(rawPaths.map((p) => repoRelativePath(path8.resolve(cwd, p), cwd)))
  ];
...
async function main() {
  ...
  try {
    ...
    log(
      `windsurf pre-tool-use: handler threw (fail-open): ${err instanceof Error ? err.message : String(err)}`
    );
    return emitAllow();
  }
